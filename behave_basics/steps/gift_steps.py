...
def step_impl(context, search_item):
    base = Base(context.browser)
    try:
        page_header = base.find_element((By.XPATH, "//div[@data-test='resultsHeading']/span"))
    except:
...
def step_impl(context, context_var, level=None):
    ...
    for header in item_headers:
        parent_elem = header.find_element(By.XPATH, "./ancestor::div[@class='sc-5da3fdcc-0 iwPybp sc-87cbc592-0 hfGzVz']")
    ...
    if param.lower() == "price":
        for item in context.table:
            ...
            if not eval(f"{price} {condition}"):
                raise AssertionError(f"Price of item '{item['item_text']}' is {price}, which does not meet the condition '{condition}'")
